defmodule AbapDump.Veil do
  @moduledoc """
  Veil's main context
  """
  alias AbapDump.Repo
  alias AbapDump.Veil.{User, Request, Session}
  alias AbapDumpWeb.Veil.{Mailer, LoginEmail}
  alias Veil.Secure
  ...
  defp new_session_url(conn, unique_id) do
    ...
    # if html? do
    cur_path = AbapDumpWeb.Router.Helpers.session_path(conn, :create, unique_id)
    # else
    # cur_path = AbapDumpWeb.Router.Helpers.session_path(conn, :create, [request_id: unique_id])
    # end
    AbapDumpWeb.Router.Helpers.url(cur_uri) <> cur_path
  end
  ...
  def verify(%Session{} = session) do
    verify(AbapDumpWeb.Endpoint, session)
  end

  def verify(%Request{} = request) do
    verify(AbapDumpWeb.Endpoint, request)
  end
